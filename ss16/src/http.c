...
static void read_request_line(struct HTTPRequest *req,FILE *in){
    ...
    *p++ = '\0';
    ...
    path = p;
    p = strchr(path,' ');
    if(!p) log_exit("parse error on line(2):",buf);
    *p++ = '\0';
...
static struct HTTPHeaderField* read_header_field(FILE* in){
	...
	log_exit("faied to read request header field:%s",
		strerror(errno));
    }
    if((buf[0] == '\n') || (strcmp(buf,"\r\n") == 0))
	return NULL;
...
static long content_length(struct HTTPRequest* req){
    ...
    if(!val) return 0;
    len = atol(val);
    if(len < 0) long_exit("negative Content-Length value");
    return len;
}
